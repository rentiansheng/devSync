...
int decoded_usr_pwd(request *in, pool_t *p)
{
	...
	if (in->rawAuthorization == NULL)
		return 0;

	base64code = string_init_from_str(p, in->rawAuthorization->ptr, in->rawAuthorization->len);
	if (string_move_word(base64code, "basic") == 0)
	{
		...
		{
			in->auth = string_init_from_str(p, dst->ptr, dst->used);
		}
	}
